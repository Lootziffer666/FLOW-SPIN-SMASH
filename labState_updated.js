...
function normalizeRuleHits(ruleHits = {}) {
  return {
    SN: Number(ruleHits.SN || 0),
    SL: Number(ruleHits.SL || 0),
    MO: Number(ruleHits.MO || 0),
    PG: Number(ruleHits.PG || 0),
  };
}

function resolveNormalizationFn(overrideFn, configuredFn) {
  if (typeof overrideFn === 'function') return overrideFn;
  if (typeof configuredFn === 'function') return configuredFn;
  return require('./pipeline').runCorrection;
}
...
function createRunStore(options = {}) {
  const configuredExecuteNormalizationFn = typeof options === 'function'
    ? options
    : options.executeNormalizationFn;
  ...
  function createRun(inputText, executeNormalizationFn) {
    const normalize = resolveNormalizationFn(executeNormalizationFn, configuredExecuteNormalizationFn);
    const result = normalize(inputText) || {};
    const corrected = result.corrected ?? inputText;
    const rule_hits = normalizeRuleHits(result.rule_hits);
    runCounter += 1;
