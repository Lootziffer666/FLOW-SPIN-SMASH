...
lines.push('| SN    | garnich, zuende, weiter-gegangen, hats, dachte-dass | Getrennt-/Zusammenschreibung, lokale Fügungen |');
lines.push('| SL    | villeicht, wier, wolte, trozdem | Silbenstruktur, Vokalfolgen |');
lines.push('| MO    | irgentwie, eigendlich, erklert, gewessen, wolte | Morphologische Stammformen |');
lines.push('| PG    | gelsen, ferig, weis→weiß, nigh→nicht, hab→habe | Phonem-Graphem-Korrespondenz |');
lines.push('| GR    | komma-nebensatz, word-repeat, sodass, apostroph-genitiv, als-wie | Kommasetzung, Wortwiederholung, Zusammenschreibung |');
...
lines.push('| Worttrennung (aus Versehen, gar nicht) | SN hat nur wenige explizite Fälle; ausversehen fehlt |');
lines.push('| Lautnahe Schreibvarianten (fehlend) | PG-Lexikon sehr klein (5 Regeln) |');
lines.push('| f/v-Verwechslung (for→vor, fergeßen→vergessen) | Kein f/v-Mapping im Regelwerk |');
